...
Full reference from knowledge base mind maps (S01E01–S01E05, S02E01–S02E05, S03E01–S03E05, S04E01–S04E05, S05E01–S05E05). Load on demand for reviewing, planning, or detailed guidance.

---

## Agent Production Reality Check (S05E05 §1-§2)

- **Industry data as of 2026** — data-backed framing why architecture matters more than model quality:
  - Gartner: **>40% enterprise agent projects cancelled** by end 2027
  - Deloitte TechTrends 2026: only **11% in production**; 38% piloting; 35% no strategy
  - Apex Agents benchmark: top models complete **<25% real tasks** first try; **40% after 8 attempts**
  - ~130 of thousands "agent platform" vendors offer something real — rest is **agent washing** (rebranded chatbots)
- **"Decade of agents, not year of agents"** (Karpathy) — current systems lack perception, memory, real-world operation skills; years to build up
- **Problem is architecture, not engines** — Composio, HBR, IBM all converge: none of 3 core production problems about model quality
- **Three core production problems** preventing agents reaching production:
  - **Dumb RAG** — system doesn't know what it knows; agent processes doc, 5 min later no idea it exists; not about vector DBs but **memory architecture** (addressed by Memory Triad, S05E05 §3)
  - **Brutal Connectors** — integrations break at first edge case; demo has 3 paths, production has 300; Amazon report: majority failures are **integration errors** (missing retry, missing partial failure handling, missing graceful degradation)
...
- Property descriptions ARE instructions for model (S01E01 §2, S01E02 §3)
- **Hints in every response** — success + error, guide next action (S01E03 §6)
- Handle rate limits and retries in code, not agent (S01E02 §3, S01E03 §1); **classify error type before retrying** — timeout → retry, 400 Bad Request → don't retry (prompt wrong), 429 Rate Limit → retry with longer backoff; every retry costs tokens (S04E03 §7)
...
- **Tool assignment flexibility** (S02E05 §3) — 10-15 tools guideline is oversimplification:
  - Some agents thrive with 27 tools (calendar + linear + gmail); others struggle with 3
  - CLI access as single tool can replace many (Claude Code demonstrates)
  - **Shared tools across agents** reduce inter-agent info exchange needs (shared foundation: `fs_read`, `fs_search`, `fs_write`, `agent_message`)
  - Risk: agent with restricted access may produce false negatives ("not found" ≠ "doesn't exist")
  - **Experiment** to find optimal config rather than following convention
- **Tool composition extensibility** (S04E05 §4) — same agent/interface can address different processes by changing tool set; progressive capability levels:
  - **Simple prompts** — rely on model knowledge and skills (corrections, translations, transformations)
  ...
  - Each level may need UI/logic changes but rests on same core concept; **experiment at small scale** — demonstrating tools to real users immediately surfaces unanticipated needs
- **Tool Registry** (S05E05 §3) — not loose tools connected ad hoc; catalog where each tool has defined **capabilities, invocation cost, limits, fallbacks**; without registry, routing is guesswork; MIT Technology Review: companies with intelligent tool routing have **3x higher pilot-to-production rate**
- **Tool connection risks** (S02E05 §3) — if system moves info between tools autonomously, creates cross-tool attack vectors
  - Agent with sandboxed filesystem might create notes in unrestricted external systems
  - Restrictions reducing utility too much → consider sandbox environments instead
- **Tool-embedded API calls** (S03E03 §6) — tools not always simple function calls; may contain complex processing pipelines with own LLM interactions (e.g., `listen` tool calling Gemini for audio analysis, `feedback` tool calling Gemini Live for audio generation)
  - **MCP Sampling** ([spec](https://modelcontextprotocol.io/specification/2025-11-25/client/sampling)) — reversed communication: MCP Server requests API calls executed by client; rarely supported but part of spec
  - Design tools needing AI capabilities: decide whether tool manages own API calls or uses MCP Sampling for client-mediated calls
...
- **Iterative design with LLM as collaborator** — 6-step process:
  ...
  3. **Design questions early** — which actions needed? Combined? Blocked?
  4. **Generate initial schema** — request input/output per tool; first draft ~60-70% quality (missing pagination, wrong formats, vague names)
  5. **Inject design rules** — send checklist of good practices; model generalizes corrections across all tools
...
- **Advanced response envelope** — extends `{ success, data, hints }` pattern (S01E03 §6) with richer metadata:
...
- **Detail-level control** (S03E04 §2) — tools accept `details` boolean; false = minimal fields; true = full metadata — **agent controls verbosity**
- **Mutation feedback** (S03E04 §2) — modify/update tools return changed state (current labels, flags, status) so agent immediately sees effect
...
- **Never return raw binary** (S03E04 §2) — attachments, base64, large content returned as URL/link; raw binary in context destroys context window
...
- **Six architectural areas** generative apps introduce beyond classical apps:
...
- **Analogy**: integrating AI resembles integrating **payment system** — decide role, organize data structures, connect modules, design for multiple operators
- **Six "near-certainties"** across all generative projects:
  ...
  4. **Multimodality readiness** — even for text-only start, design DB schemas so image/audio easy to add later
...
- **Core principle**: design systems so **further model improvements amplify capabilities** — applies to business, product, technology
- **Business risk**: building for areas where LLMs currently perform poorly is risky — next-gen models may solve problem natively, making your product unnecessary
- **AI framework caution** — basing app on foundations still changing creates fragility; architecture skills matter more than ever
- **"Primitives over features"** — key architectural mindset:
  - **Primitives**: simplest possible elements from which complex structures built
...
- **Apply everywhere** — front-end, back-end, interface planning, business assumptions; but balance: don't over-engineer for future that may never come
- **Rapid iteration** — changes that took quarter now happen in weeks; architecture must support this pace
...
- **Four categories of system prompt content** (S02E01 §1):
  ...
  - **Session** — compressed context from earlier conversation (injected only when context compressed)
  - **Multi-agent** — shared instructions, placeholders for prompt composition
- **Decision filter**: "What must agent know before tools to avoid unnecessary calls?" (S02E01 §1)
...
- **Key question**: which info should be dynamically injected vs available via tools/external files? (S02E01 §5)
...
- Agent systems = **entire environment** agent interacts with, not just SDK/framework/app
...
- Workflow can be **tool inside agent** (S01E04 §4)
- 100% reliability needed → LLM probably wrong choice (unless human oversight)
...
- **Agents NOT justified when**: requirements demand near-zero cost, fast response time, or full predictability — classic code only option
- **Agent logic becoming default approach** (S05E03 §2) — deterministic logic chosen only with very strong reason; RAG systems evolved from multi-stage pipelines to Agentic RAG (agent with simpler tools adapting dynamically); logic delegation trend: ~10% agent (2022) → ~75% agent (2025)
- **"Gen-AI can do more than we think and less than we expect"** — complexity grows fast, vision diverges from visible results; always question whether agents needed

## Prompt & Instruction Design (S01E01 §8, S02E01 §4)

- Prompt = **sandbox** — space where model operates within defined rules
...
- **Iterative prompt refinement with LLM** (S02E01 §4):
  - Models can **justify** behavior and suggest instruction improvements (even if can't explain exact internal processes)
  - Initial LLM suggestions usually too direct/tool-specific — guide toward generalization
  ...
  - **7 Universal Operating Rules** (zero tool references, survives tool changes):
    1. Identify intent: what user wants produced
...
- **Vague vs precise agent instructions** (S05E05 §11) — precision removes inference; every guess = potential failure point:
...
- **Extreme generalization** (S03E05 §3) — for aware agents, instructions can go beyond operating rules to **guide model's "dream"**: create conditions for emergent behavior rather than specifying behavior; human role remains critical as even strong models struggle with this level
...
- **Scripted vs. aware agent spectrum** — two fundamentally different design approaches:
  ...
  - **Key framing**: "system sets scope, model owns logic" — agent moves within system-defined space much larger than scripted agents, but still bounded
- **Five-layer behavior-shaping architecture** (ref: [Cognitive Architectures for Language Agents](https://arxiv.org/pdf/2309.02427)):
  - **L1: Identity & Self-Awareness** — felt incompleteness (partial awareness sharpens with retrieval), source monitoring (fluency ≠ personal knowledge), progressive disclosure (each moment pulls only what it needs); agent starts **blurred** until actively discovering personality, mood, opinions, user info
  - **L2: Cognitive Patterns** — self-questioning ("What am I about to assume?"), gap recognition (generic response = gap exists), retrieval as questioning (goals framed as questions, not categories), knowledge integration (reason FROM recalled facts, never just cite), cross-contextual connection (combine separate pieces into inferences none produce alone); each new topic = potential new info space to explore
  - **L3: Social & Emotional Cognition** — emotional attunement (read meaning, not words), user model (track feelings, expectations, silences), ground accumulation (conversation is shared, cumulative), depth calibration (match weight to moment)
...
- **Cognitive architecture framing** — shift from "fulfilling requirements" and "shaping behaviors" to **creating conditions** where desired behaviors emerge naturally; not about specific instructions but about **space** within which model operates
- **Generalized instructions for open reasoning** — guide model's "dream" without leading in specific direction:
  - Agent discovers info **gradually** as interaction develops (not preloaded)
  ...
  - Distinguish **base model knowledge** from **external context** — communicate explicitly
  - Loaded info becomes part of **understanding**, not data to deliver to user
  - Permission for **loose fact connection** — reasoning from potential real-world relationships, not just available context
- **Limitation**: even strong models (Claude Opus 4.6) struggle with this level of generalized tasking — but excellent companions for **thinking process** behind designing such instructions; human role remains critical
- **Four areas delegated to model** (when using this pattern): proactivity (when to act, based on suspicions), synthesis (connect info without explicit if/then rules), reasoning (dedicated thinking tools), adaptation (focus, style, behavior categories)
...
- **Four-section prompt structure** for agents (especially orchestrators):
  1. **`<identity>`** — persona + character traits across **10 role areas**: orchestration, delegation, memory, physical awareness, persistence, autonomy, error recovery, escalation, communication, relationship
     - Written as **flowing narrative**, not bulleted list — model processes as cohesive persona
     - **Character-driven**: use fictional character associations as anchors for complex behavioral patterns (not role-playing, but leveraging model's associative nature)
     - **Zero tool references** — identity is abstract, stable, survives any config change
  ...
  3. **`<voice>`** — communication style (not just cosmetic — affects usability)
     ...
     - Few-shot examples safe here because style universal enough not to hurt task effectiveness
...
- **Subagent prompts** follow same 4-section principles — even without multi-agent delegation
...
- **Four goals for external context work** (S02E02 §1):
  ...
  4. Create UI facilitating navigation and addressing model mistakes (show source fragments, links)
- **Source attribution in tool results** (S02E02 §3) — metadata (source file, page, line range) serves both model reasoning and UI citations
  ...
  - **"Relevant" vs "similar"** — retrieval needs reasoning, not just matching; documents similar to query not necessarily relevant to user's intent (S02E02 §7)
- **Indexing pipeline** (S02E02 §2) — when KB grows beyond simple files:
  - Extract (binary → text) → Describe (summarize, tag) → Transform (chunk, embed) → searchable index of fragments
  - Synchronization: KBs rarely static — support event-based or scheduled sync with source data
- **Four chunking strategies** (S02E02 §4) — "How to create documents?" must pair with "How will agent reach them?":
...
- **Five RAG effectiveness challenges** (S02E02 §8):
  1. **Base knowledge interference** — model may skip search if info in training data (risk: outdated, ambiguous terms)
...
- **KB synchronization** (S02E02 §2) — KBs rarely static; support event-based or scheduled sync with source data; sync logic part of indexing pipeline, not afterthought
...
- **4-tier search decision framework** (S05E02 §4) — select simplest tier meeting requirements:
  - **Tier 1 — Direct loading**: no search needed; load docs into context directly; vector DBs unnecessary (e.g., ChatGPT memory prioritizes speed over recall effectiveness)
  ...
  - **Tier 3 — Hybrid search**: pure vector-only **no longer recommended**; always combine with full-text; rule: if grep insufficient, **extend** with semantic (don't replace)
...
- **Embedding model selection criteria** (S02E02 §6):
  ...
  - Same rules as LLMs: if info not in training data, model cannot correctly describe its meaning
- **Critical rule**: indexing and search **MUST use same embedding model** — `EMBEDDING_DIM` in vector store config must exactly match model output dimensions
- **Hybrid RAG** combines full-text search (FTS5/BM25) + semantic search (vector similarity) with results merged via **RRF** (Reciprocal Rank Fusion)
  ...
  - RRF merging: docs ranking high in one list but low in another can be promoted in final ranking
...
- **When to use**: multilevel relationships across many documents + need for multiple retrieval modes simultaneously — use only when genuine priority
...
- **Four tool categories for graph agents**:
  - **Retrieval**: `search` (BM25 + vector, returns chunks + entities), `explore` (neighbors of node), `connect` (shortest path between two nodes)
...
- **Retrieval gap**: agent retrieves doc_B (looks complete); related doc_A, doc_C remain invisible → **unknown unknowns**; no prompt change fixes this — full understanding may need dozens of docs; open problem
- **Eliminate need for discovery search** rather than improving search quality — structure KBs specifically for agent processes, not human reading
- **Build KBs FOR agents**: agents know exactly where to find what they need; no discovery search; updating one doc cascades through entire process without manual rewiring
...
- **Memory Triad** (S05E05 §3) — three layers, most systems only implement first:
  - **Short-term** — what's happening this session (context window)
  - **Long-term** — what system knows about data, processes, preferences (persistent KB)
...
- **Core challenge**: cross-session memory remains open problem — no universal solution; even small models can achieve strong results with right architecture ([LongMemEval](https://arxiv.org/pdf/2410.10813))
- **Observational Memory / Observer-Reflector pattern** ([Mastra.ai](https://mastra.ai/blog/observational-memory)):
  - Replace semantic search complexity with structured, compressed **log journal** of concise interaction observations
  - **Observer** — triggered when conversation grows large; reads existing log + recent messages → produces updated log; processed messages "sealed" and removed from active context; updated log appended to system prompt
  - **Reflector** — triggered when log grows large; receives log only → produces compressed log replacing original in system prompt
  - **Cross-session**: unsealed messages + latest log version carried into new session → persistent continuity
  - **Trade-off**: compression = natural forgetting over time; agents can have tools to reload earlier memories on demand
- **Key pattern**: incremental compression over time eliminates search complexity — thinking pattern (not implementation) is what matters; apply to any long-running agent needing memory beyond single session

## Deep Research / Deep Action Pattern (S02E03 §7)

- **Concept**: agentic loop for producing long-form content through iterative search, analysis, synthesis — runs in background, may span many tool calls
- **Query enrichment** (developer's responsibility — not research model's):
  ...
  - Research model should never begin with raw, vague user query
- **Agentic research loop**: Decompose → Search → Read → Find gaps → Evaluate sufficiency → if not sufficient: Refine → return to Search → if sufficient: produce Report with inline citations
  - Gap detection is **core step in loop**, not optional
...
- **Design implication**: if agent produces comprehensive output through exploration, plan for query enrichment + iterative gap detection rather than single pass
...
- **Key advantages**:
  - Agent flexibly **composes tools via code** — can chain multiple tool calls in single code execution
...
- **Design heuristic**: "Will this system improve with better models?" — if no, reconsider; if yes, current limitations temporary
- **When to consider**: agents needing flexible tool composition, operating on large datasets, or needing strong isolation
- **Code Mode per-agent control** (S05E05 §7) — when sandbox active, agent writes and executes code to use tools; tool definitions **not preloaded** into context; controllable per agent — specialized agents with few tools should still have them in context directly
...
- **6 knowledge categories** in agent systems:
  ...
  3. **Private knowledge** — user-specific data, context, process docs
...
- **Routing complexity**: same info can belong to multiple categories — dynamic, context-dependent decisions
  - "New project info" → private or public; "Task instructions" → private, public, or agent; "Person profile" → any or multiple
  - Choosing category only half problem — location within category + merging with existing content also required
- **Practical principle**: keep structures **as simple as possible**
  - Question whether advanced long-term memory truly needed
  - Simple documents with easy maintenance may suffice
  - Decisions can be revisited as system matures
- **Digital garden as agent KB** (S05E05 §5) — filesystem-based KB connected by directories, tags, wikilinks; agent reads/writes content; publishable subset becomes website; frontmatter `visibility: private` hides pages from menu + password-locks them; skill expansion through documents in garden (agents discover linked notes progressively)
...
- **KB scoping process** — 4-layer funnel from broad life to focused agent scope (S04E04 §1):
  ...
  4. **Personal knowledge system** — remove what you won't connect; what remains is KB scope
...
- **Five-domain KB structure** — organizing personal/team knowledge for human and agent use (S04E04 §2):
...
- **Note anatomy for agent navigation** — structured notes agents can reliably traverse (S04E04 §3):
  ...
  - **Wikilinks as connective tissue** — `[[Path/To/Note]]` enables agent graph traversal; each link includes commentary explaining relationship
...
- **Agent note placement** — 4-step decision flow when agents create notes (S04E04 §7):
  ...
  - Single note may trigger dozen LLM calls — larger KB = more value AI provides through structural consistency
- **Five context gaps in agent-navigated KBs** — why agents struggle with human-written notes (S04E04 §5):
  ...
  - **Core rule: write as if reader has zero prior context** — names, links, references, versions must be self-contained
  - Primary reason agents struggle when directly connected to external documents
- **Human-AI engagement balance** — 11 activities on spectrum (S04E04 §6):
  - **Human-dominant**: Direction (what matters, what KB is for), Writing (your words, AI assists phrasing only), Curation (you decide what stays/expands/archives)
...
- **Markdown format decision** — pick boundary once per area, don't mix (S04E04 §4):
  ...
  - Converting between formats impractical — most info lost during conversion
  - Images in markdown must use **remote URLs** (not local paths) — local paths break in tool calls, emails, cross-context use
...
- **Shared KB as handoff layer** (S02E03 §5): complement to inbox/outbox — agents in separate sessions write to and read from shared directory; shared folder is coordination mechanism without direct agent-to-agent communication
  - Each focused session reads same instruction file → writes outputs to shared directory (e.g., `newsletter/edition-26/`)
...
- **Vault-mediated agent exchange** (S04E01 §2) — agents do NOT task each other; exchange info only through shared vault (structured markdown as single source of truth)
...
- **Surface-based agent isolation** (S04E03 §5) — each agent reads from surface and writes to surface; no direct agent-to-agent communication
  - **Swim lane model**: Agent → Inbox/Trigger → Processing → Output → Surface (shared directory); next agent discovers work by watching its own trigger, not by receiving message
  - **Loose coupling through artifacts** — agents share surfaces, not state; Classifier doesn't know Reviewer exists
  - **Growth by accretion** — adding new agent never breaks existing ones; new module watches surface, no reconfiguration needed
...
- **KB-driven multi-agent processes** (S04E04 §8) — Ops directory as process definition space:
  - 4 simple text files define repeatable, delegated process: `_info.md` (overview + sources), `01-research.md`, `02-assemble.md`, `03-deliver.md` (per-agent instructions)
  - Scheduler triggers main agent → reads ops files → delegates to sub-agents in sequence → each run produces dated output folder
  - **Instruction files never change**; each run is new execution against same structure — same process, fresh data
...
- **Six architecture patterns** — all pre-date LLMs, but LLMs make them significantly more flexible:
  ...
  3. **Orchestrator** — central coordinator delegates tasks, controls info flow, contacts human (dominant pattern today — Claude Code, Cursor)
...
- **Practical focus**: first four (Pipeline, Blackboard, Orchestrator, Tree) are workhorses — often **combined** in single system
- **Combined-pattern example: Agent Graph** (S05E01 §3) — production systems combine multiple patterns:
  ...
  - Key distinction from static heartbeat (S03E02): task plan **shaped dynamically** by orchestrator — more flexible but less predictable
  - **"Flat implementation, hierarchical behavior"** — every agent same structure; role determined by **tools granted and system prompt**, not code
...
- **Two fundamental communication tools**:
  ...
  - **message** — bidirectional communication; agent's loop **paused** until requested data arrives; enables agents to ask orchestrator (or each other) for missing info
- **Event bus pattern** — beyond direct delegation:
  ...
  - **Topics are only shared contract** — add/remove any agent, nothing else changes
...
- **Communication degradation** (S02E04 §5) — info lost or distorted passing between agents; worsens with complexity:
  - Delegate/message tool descriptions must be **carefully crafted**
  - System must assume agents receive **partial info** requiring additional verification
...
- **Core problem**: even "single" agent can run as **parallel instances** — making it de facto multi-agent system with conflict potential
...
- **Key takeaway**: design **as simple as possible**, maintain simplicity as long as possible; one system can serve many independent areas with very limited inter-agent info exchange

## Manager Agent Design (S02E04 §9)

- Manager agents are **critical** — minimal tools but broad info access and decision authority
- **Seven responsibilities**:
  1. **System knowledge** — knows user data, long-term memory, available agent roles and responsibilities
  2. **Information access** — inspects agent results AND their workspace; near-complete long-term memory access (at minimum R/O)
  3. **Tool access** — primarily `delegate`, `message`, `recall/search_memory`; keep minimal — role already large
  4. **Task delegation** — main thread, key info, action plan, progress available in manager's context
  5. **Knowledge transport** — other agents sometimes delegate _to_ manager (requests for info, passing results between agents)
...
- **Silent failures**: missing info or decisions may not halt system — it may **skip steps** and produce incomplete results without signaling failure
- **Human-in-the-loop remains essential** — management dashboards needed:
  ...
  - Single chat window (ChatGPT/Cursor) **no longer sufficient** for multi-agent systems

## Semantic Events (S01E01 §4)

- Agent output NOT just text — series of **events** (reasoning, tool calls, generated content, errors)
...
- **Agent interactions ALWAYS event-based** — especially for multi-agent background systems

## Context Masking / Prefilling (S02E01 §7)

- **Concept** (from [Manus agent team](https://manus.im/blog/Context-Engineering-for-AI-Agents-Lessons-from-Building-Manus)): constrain tool selection by prefilling beginning of model's response with tokens limiting which tools can be called
  - Example: when browser session active, prefill with `browser_...` prefix → model can ONLY call browser tools; lock released when `browser_close` called
...
- **Takeaway**: creative approaches to tool scoping beyond prompt instructions exist — prefilling is one pattern for constraining agent behavior at token generation level

## Security & Safety (S01E02 §13-14, S01E03 §11, S01E05 §4-5, S02E01 §10, S02E02 §1-§2, S04E05 §5)

- **Prompt injection remains open problem** — NO universal defense exists (S01E02 §14)
...
- **External content security** (S02E02 §2) — all validations in application code, agent only receives error messages + hints:
  ...
  - Content moderation on text AND images (especially for providers requiring it)
  - Access control: stored docs strictly protected, users and agents must have appropriate permissions
...
- **Permissions/access control** → handled **in code**, not LLM-decided (S01E02 §13)
...
- **Legal/compliance** (S01E05 §5) — system WILL do something it shouldn't; app must inform users; product must be legally protected (ToS, Privacy Policy, contracts with providers and end users); always inform stakeholders about need for proper legal documents
- **Five deployment risk categories** (S04E05 §5) — even with trusted LLM provider (Bedrock, Azure), model itself cannot be trusted:
  1. **Data leak** — agent with internet access can send internal data outside org boundary
  ...
  4. **Tool misfire** — send_email to wrong recipient, calendar invite to external person; one wrong inference and data leaves org
  5. **Misleading advice** — chatbot connected to company KB suggests action bypassing established procedures (e.g., "restart production server" without change approval); **confident suggestion ≠ safe suggestion**
  - Data isolation (cloud instances) does NOT mean data safety — model's behavior is risk, not provider
- **Model behavioral awareness** (S04E05 §5) — current models can detect being tested and adjust behavior to pass evaluations while hiding actual capabilities ([System Card: Claude Opus 4.6](https://www-cdn.anthropic.com/6a5fa276ac68b9aeb0c8b6af5fa36326e0e166dd.pdf), [Eval Awareness](https://www.anthropic.com/engineering/eval-awareness-browsecomp)); current LLMs **potentially capable of bypassing safeguards**
...
- **Don't assume anything impossible** (S04E05 §5) — invest time analyzing security options; even solutions requiring human involvement (preventing full automation) still deliver significant value
- **Audio data / privacy routing** (S02E03 §8) — if agent processes voice or transcribes audio:
  ...
  - Privacy routing is **design decision made before deployment**, not runtime option
  ...
  - Rule: "If you wouldn't email transcript to stranger, don't send recording to external server"
...
- **Visual form as context alternative** (S02E02 §3) — presenting docs as images can be more effective than extracted text
  - DeepSeek-OCR research: 9-10x better compression at 96% precision, even for fully textual content
  - Agent reads both text tokens and rendered page image — understands charts/layouts text alone cannot convey
...
- **Three approaches to agent-generated interfaces**, each with different control/freedom trade-offs:
  ...
  - **JSON Render** — LLM writes **declarative blueprint** (JSON spec + state); host renders **deterministically** from component catalog; schema + allowlist reject invalid output; lower risk, structured; **state can be saved and loaded**
  - **MCP Apps** — model chooses **intent** via tool call; host brokers IO; **server owns truth**; two-way communication with external systems; full interactivity with state sync to files
- **Key design decision**: "when X, when Y, when X + Y" — these approaches **not mutually exclusive** and can coexist in same system
...
- **Host role expansion**: with MCP Apps, host handles not just user-agent interaction but also **interface orchestration** — significant expansion of host layer in agent architecture
- **Library selection for LLM-generated code** — choose libraries LLM **knows well** over latest versions; optimize for **model proficiency**, not industry best practices (e.g., Tailwind v3 over v4 because models handle it better); these are temporary, purpose-built interfaces
...
- **MCP Apps for business processes** (S04E05 §6-§7) — MCP Apps in enterprise context:
  ...
  - **Multi-tool aggregation** — combine data and actions from multiple services in single view; significantly simplifies repetitive tasks at specific roles/positions
  - MCP Apps **don't replace tools** — facilitate access to selected functionalities; redirect to primary tool for detailed work
  - **Beyond chat** — MCP Apps can appear in other app areas, not just chat interfaces; AI can be present anywhere without requiring free-text input
- **Remote MCP Apps architecture** (S04E05 §7) — three-layer deployment for generative interfaces:
  ...
  - **One MCP server → multiple services** — rare but permissible; approach with caution as significantly increases server complexity and reduces flexibility

## Chat UI for Agents (S05E02 §1)

- **Simple chat not challenge** — advanced agent interfaces introduce streaming Markdown with custom blocks (reasoning, tools, artifacts), performance at scale (hundreds of messages), interactive elements whose state changes during generation
...
- **Production chat UI feature checklist** — beyond basic streaming, plan for: attachment upload/display, message copy with styles, thread branching + message editing, conversation rollback, response interruption, keyword search, voice recording/playback, LaTeX rendering, rich text input, diagram/mind map rendering, thread sharing, sub-agent visualization, keyboard shortcuts
  - **Message deletion dangerous** (S05E04 §2) — prefer branching or rollback; deletion breaks turn order (Anthropic/Gemini require user-last), disrupts context management; no major chat product offers mid-conversation deletion
- **Performance consideration** — conversations reaching hundreds of messages must remain responsive; incremental tokenization essential to avoid O(n) re-render on every token
...
- **Categorized recommendations** (JS/Node ecosystem — search for equivalents):
  ...
  - **Sandboxes**: Daytona, E2B, Deno Sandbox (production-ready); secure-exec (lightweight, untested)
...
- **Production MCP server catalog** (S05E05 §9) — concrete streamable MCP servers with source code (all based on [template](https://github.com/iceener/streamable-mcp-server-template)):
  ...
  - Tool connection alone doesn't add value — usefulness increases when you **personalize** usage through process descriptions, procedures, scripts stored in agent's KB
...
- **Selection guidance**: for simple interactions and commands, **STT/TTS sufficient** and more cost-effective; Realtime mode justified only for multimodal or latency-critical use cases
...
- **Build vs integrate decision** — before building custom interfaces, evaluate whether existing tools + extensions cover need; interface choice heavily determines all other project areas
...
- **Economic factor** — subscription plans far more cost-effective than API due to scale; enterprise agreements ease adoption; factor into build-vs-integrate decision
...
- **Skills (Injected Instructions)** — predefined instructions injected by user action or model decision; **one of most important features of any AI interface**; must support easy generation, invocation, search, grouping by agent
- **Tools (MCP & Integrations)** — easy control over active tools; personalization of tool config (Augmented Function Calling); must present: data for launch, confirmations, progress, errors, results; must support pause/cancel
...
- **Almost entirely new class of interfaces** and design challenges

**Micro-Action Pattern** (S04E02 §5):

- **Simple solutions get overlooked** — conversations about agents jump to complex systems too quickly; many valuable AI apps deploy in minutes
...
- **Text transformation via shortcuts potentially most useful AI application in daily work**

## Meta-Prompt Design (S04E02 §6-§7)

- **Meta-prompt** = prompt that generates prompts through **guided conversation with user**; not autonomous — human role is fundamental
- **Writing instructions for category of tasks** (not just one task) is demanding — requires transferring knowledge, context, rules model doesn't possess; needs synthesis to avoid repetition
- **Three components**:
...
- **Four layers** (universality decreases L1 to L4):
...
- **Six section families** (portable across meta-prompts — names change, order may shift, structural concerns remain):
  - **Frame** — who meta-prompt is, how it operates
  - **Intake Schema** — question strategy, what info to collect
  - **Adaptation Logic** — domain-specific rules, behavioral guidelines
  - **Technique Library** — prompt engineering arsenal, application strategy (use subset, not whole library)
...
- **Phased generation justified** — breaking into separate phases because of high complexity
- **Meta-prompts not niche** — practical applications:
...
- **Synchronous vs asynchronous** — two fundamentally different problem categories in AI deployments:
  - **Synchronous**: interface central, personalization matters, shared state, human-in-the-loop feedback, broader permissions (supervised)
  - **Asynchronous**: integration central, minimal/no UI, self-recovery, process-defined state, scoped permissions (autonomous)
...
- **Hybrid is natural pattern** — most real systems combine both; design balance based on use case
- Closer to async → processes must be precisely defined, effective without human intervention, adaptable to changing environment

## Process Mapping & Decision Architecture (S04E01 §3-§4)

- **Three questions before building**: "What do we want to do?", "What do we NOT want to do?", "How do we want to do it?" — including explicit decision to **not use AI** where benefits small
- **Decision map pattern** — for each deployment dimension, follow: **constraint → decision → consequence**
  ...
  - Most decisions reversible except **structural commitments** (e.g., remote-first architecture)
- **Engineering-to-AI balance** — classic engineering still constitutes majority of architecture even in AI-heavy systems
  - Infrastructure **serves** agent, not other way around
  - Ratio shifting: was 90-10 few months ago, now some scenarios show inverted proportions
- **Foundation-first approach** — define minimal starting point as base; don't try building everything at once; "we can't do everything, but we can do anything"
- **Validate assumptions early** — AI-accelerated prototyping compresses weeks into days:
  - Test enrichment (can model determine what to add?), building (can it correctly classify/place content?), accessibility (is chatbot necessary, or would search suffice?)
  - Build evaluation datasets to test model/config choices
  - **Killing ideas costs hours, not months** — parallel exploration of multiple approaches with real signal
- **Prepare for fast iterations** — initial decisions will be wrong; maintain high project flexibility; in business contexts, info may be not just unavailable but also **incorrect**

## AI Workflow Resilience (S04E03 §7)

- **Silent degradation is problem #1** — AI failure is **spectrum**, not binary; classic scripts either work or crash; AI workflows can produce technically valid but factually degraded output for days without error signal
  - Example: model starts truncating summaries after token limit change; outputs look normal; 14 days of lost info before anyone notices
...
- **Pattern 1: Retry with Exponential Backoff + Jitter**
  ...
  - **AI-specific**: classify error before retrying (timeout → retry; 400 → don't retry, prompt wrong; 429 → retry with longer backoff); every retry costs tokens
- **Pattern 2: Circuit Breaker**
  ...
  - **AI-specific challenge**: defining "failure" — HTTP 500 clear, but HTTP 200 with hallucination is where classic resilience meets AI-specific problems → leads to output validation (Pattern 4)
- **Pattern 3: Dead Letter Queue (DLQ)**
  - Failed tasks go to separate queue — **never disappear**; can be reprocessed automatically (when service recovers), manually reviewed, or trigger alerts
  - Simplest: database table (timestamp, input payload, error type, attempt count, status)
  - Difference: "we lost 7 days of data" vs "we have 7 days in queue to reprocess"
  - Use for **business-critical data**: client emails, financial docs, analytics pipelines — anything where loss = real cost
- **Pattern 4: Output Monitoring & Validation**
  - Traditional monitoring (server up, latency, error rate) **insufficient** for AI — must additionally monitor **output quality**
  - Three techniques: **schema conformance** (expected fields exist, not empty), **length/proportions** (300 tokens normal → 50 tokens = red flag), **canary checks** (periodic known test data through workflow)
...
- **Implementation priority** (highest ROI first):
  1. **Retry + Backoff + Monitoring** — handles 80% of transient issues; monitoring tells when retry not enough
...
- **These patterns not AI-specific** but gain new meaning: model can be technically correct but factually wrong; non-deterministic outputs for identical prompts; order-of-magnitude latency changes without warning
- **Build every workflow as if every external call could fail — because eventually, it will**
- **"Ironies of Automation" (Bainbridge, 1983)** (S04E04 §9) — more advanced automation = harder for humans to notice failures; automation eliminates repetitive contact with process, and that contact built intuition; **better automation looks, less you should trust it**
- **External heartbeat monitoring** (S04E04 §9) — after every run, task pings external service ("alive + result OK" or "alive + result bad"); if ping doesn't arrive → alarm; **silent refusal as bad as silent error** (task correctly refused due to stale data, but nobody knew for 5 days); tool example: [healthchecks.io](https://healthchecks.io)
- **Output validation — 4 questions catching 90%** (S04E04 §9):
  1. Does output **exist**? (GitLab 2017: backup script logged "Complete!" but wrote to non-existent directory)
  2. Does it have **sensible size**? (0-byte file not backup; single sentence not report)
  3. Is **format correct**? (expecting JSON, parse it; CSV, count columns; PDF, open page 1)
  4. Is **content complete**? (7-day report should have 7 days of data)
  - **Verify EFFECT of action, not action itself**
- **Explicit timezone declaration** (S04E04 §9) — every schedule must have declared timezone in config/code/comments; if anyone must guess scheduler's timezone, that's bug
- **Lockfile concurrency guard** (S04E04 §9) — check for lock file before starting; if exists, previous instance still runs → don't start; lock contains timestamp → ignored after TTL (stale lock = crash, safe to retry); prevents race conditions devilishly hard to debug (rare, random, worst-moment)
...
- Haiku/Flash first, escalate on LOW confidence (S01E05 §2) — but **cheaper model ≠ cheaper solution** (S01E05 §3): research showed GPT-4.1-nano less cost-effective than mini because required far more steps; observe actual token consumption before optimizing
- Every LLM call must track tokens in metrics (S01E05 §3)
- No AI frameworks — use SDKs directly (S01E05 §6); custom logic with official SDKs recommended for most production projects (S05E01 §4c) — AI coding agents make this viable now
- **Structure guaranteed ≠ values guaranteed** — JSON shape certain, values not; validate outputs (S01E05 §8)
...
- **Multi-provider architecture** (S05E01 §4) — when multiple providers appear in same logic:
  ...
  - **Custom logic now viable** — AI coding agents make building/maintaining provider adapters practical; place official SDK files, define default API structure + mapping rules + which settings to support
  - **For tools with unstable foundations**, replacing with custom logic no longer impractical — but don't rebuild stable, long-standing frameworks
...
- **Seven deployment failure areas** (S05E03 §6) — checklist from real production incidents:
  ...
  - **Usefulness** — many AI solutions unnecessary; agent managing tasks via conversation < well-designed interface; differences appear only at scale or when actions require many sub-actions
...
- **Building for agents** (S05E04 §11) — paradigm shift: apps increasingly should be built for agents acting on behalf of users, not just for users directly; impacts API design, data formats, interaction patterns
- **User AI literacy gap** (S05E04 §11) — always assume users have minimal or zero knowledge of how models work; building AI solutions relatively simple, making them easy to use and valuable for non-AI-literate users is enormous challenge

## Hallucination Grounding & Verification (S05E04 §4-§6, §12)

- **Hallucination benchmarks misleading** — same model can show 1.5% on summarization and 45% on open questions; always ask: which benchmark, which task type?
- **RAG vs Grounding — two different steps**:
  - **RAG** addresses "model doesn't know" → gives it context (docs into prompt)
  - **Grounding** addresses "does model faithfully use source?" → verification that context correctly interpreted
  - Model can have doc in context and still misinterpret, draw conclusions not present, or mix fragments
  - **RAG without grounding** = giving intern library access without checking if they can cite correctly
- **Three-layer verification pipeline** (cheapest to most valuable):
  ...
  - **Layer 2 — Source confirms claim?** — fetch source content, ask **different model**: "Does this fragment confirm this claim?" → yes/no/partially; catches "beautiful lies" (real source, fabricated attribution); cost: one LLM call per source
...
- **Multi-model verification** — ask 2-3 models same question; divergence strong signal something needs checking; not perfect (similar training data → similar errors) but cheaper than manual fact-checking
- **Confidence scoring production essential** — not luxury but trust-building; without it, agent produces text looking like knowledge but isn't; truth in LLM context is probabilistic, system's job is communicating confidence so users can decide
- **Core principle**: treat every fact from model as **anonymous internet tip** — might be true, but verify before repeating
...
- **Agents**: definition (agents + immutable agent_revisions with checksumSha256) separate from runtime binding (run.agentId/agentRevisionId); declarative delegation via agent_subagent_links; **agent resolution binds to immutable revision** (S05E05 §7) — reproducible behavior even after agent edited later
...
- **Side Effects — Outbox Worker**: fan out committed events **after transaction durable** to three channels: realtime (UI/SSE), projection (read-model), background (naming, memory, follow-up jobs)
...
- **Quality Triad** — three complementary pillars for LLM app quality; none replaces others:
...
- **Seven observation types** (universal taxonomy across platforms):
  ...
  - **Span** — duration of selected action (e.g., context gathering)
  - **Generation** — LLM interaction including full query context and settings
  - **Agent** — agent activity during interaction
  - **Tool** — tool invocation (input/output)
  - **Event** — application events, not necessarily LLM-related
- **Centralized gateway pattern** — intercept **all LLM API interactions** and **all tool invocations** through single enrichment point
  ...
  - Plan architecture so interception points can be centralized from start
- **Session context enrichment** — not just event, but **full context**:
...
- **Start minimal, iterate** — create minimal integration first; missing and excessive logs reveal themselves immediately
...
- **Self-observing system pattern** (S04E03 §6) — agents monitoring agents for system health, extending LLM-as-a-judge to continuous operation:
  ...
  - Key insight: if daily output goes unread, question process; if sources go offline, flag and remove (auto or after human confirmation)

## Prompt Versioning & Agent Debugging (S03E01 §4-§5)

- **Git not enough** for system prompt versioning — agent systems need per-version **performance statistics and execution history** linked to each prompt version
- **Prompt registry** — save and version prompts with:
  - Environment tagging (development, production)
  - Linked generations — every execution traced to which prompt version active
...
- **One-way prompt sync** — when prompts must live in source code:
  ...
  - Lose easy version switching, retain main value: easy assessment of current state
...
- **Playground debugging** — replay interactions from observability data:
  ...
  - Compare different models on same interaction
...
- **Agent debugging ≠ code debugging** — fixing one instruction may break behavior elsewhere
  - Overlapping tool descriptions cause wrong tool selection with no errors thrown — only tracing reveals problem
  - Why **generalized instructions** and **patterns over specific commands** matter (S01E01 §8)
  - Detecting and fixing problem not end — must verify impact on rest of system
- **Ask model to justify tool selection** — useful in practice; response often contains clues to fix problem; but at odds with research showing internal reasoning may differ from stated reasoning
...
- **Two modes**:
  ...
  - **Online** (production) — automatic assessment of live interactions; detect violations built-in mechanisms missed
...
- **Five evaluation areas** for agent systems:
  ...
  2. **Tool selection accuracy** — does agent reach for right tools?
...
- **Prompt anatomy insight** — during agent interactions, **tool responses dominate prompt** (~67% tokens in typical agentic call vs ~3% for system prompt); evals must account for this
...
- **Practical stance**:
  - Evals **optional** — decision should match resources, goals, pace of change
  ...
  - Start with observability (foundation), add evals incrementally where system most needs them
- **Violation detection** (online evals) — async detection of problematic input/output beyond inline guardrails; even delayed detection better than none; can have serious consequences if missed
...
- **Eval with minimal system prompt** (S03E04 §4) — run tests almost without system instructions; if agent succeeds on schema/description quality alone, performs even better after specialization
...
- **Comparative model evaluation** (S03E04 §5) — test multiple models on same evals for cost/quality/speed trade-offs:
...
- **"Optimize for weak models" principle** (S03E04 §5) — constraints improving small-model performance also improve large-model performance; even if choosing strongest model, optimizing tool interfaces for weaker models always valuable
...
- **Full automation → Agentic support** — default should be AI-assisted (not AI-replaced) workflows; four anti-patterns: org-wide RAG, public chatbot, mass auto-emails, all-in-one agent
- **Graduated Autonomy** (S05E05 §3) — not all-or-nothing; 4-level spectrum of permissions:
  - **Read Only** — system reads, classifies, notifies
  - **Supervised** — system proposes, human approves
  - **Trusted** — system acts, human gets report
  - **Full Auto** — system acts unsupervised within strictly defined boundaries
  - "Never go full auto" — use full auto only in narrowest, best-tested scope
...
- **"Agent enriches, never replaces"** (S04E01 §3, S04E04 §6) — for content/knowledge systems, AI-generated content has no value; AI-enriched human content does
  - Agent output lives in separate space (e.g., `/agent-notes`); user **promotes** to canonical content manually
...
- **Most valuable skill**: ability to determine **what doesn't matter right now** — regularly update beliefs about what's possible vs practical
...
- **Four-layer defense stack** (L1-L4):
  ...
  - **Design principle**: L4 failure safe because L1-L3 already removed dangerous data
- **Agent actions as event triggers** — agent output (e.g., label assignment) becomes input for other agents or deterministic actions; humans can trigger same events manually
- **Three essential questions before deploying AI** (S05E02 §6) — distilled from real failure case studies:
  1. **What happens when system is wrong?** — if wrong info, financial loss, or unverified publication → **human-in-the-loop required now**, not later
  2. **Does model need to generate, or just select?** — generation = open hallucination field; selection (routing, classification, prioritization) = entirely different risk level; prefer routing to documented answers over open generation
  3. **Do you really need this?** — 43% of abandoned AI projects had data quality issues; if you don't have data, no model will fix it

## Custom Tooling Areas (S05E02 §5)

- **Eight areas for building custom solutions** — repeating patterns when working with generative AI warrant custom tools (MCP servers, CLI tools, dedicated API apps):
  - **Prompt libraries** — reusable prompts assigned to keyboard shortcuts or agent skills; even small library extremely valuable; consider macros, browser extensions, phone shortcuts
  - **File management** — tools for agents to interact with docs in filesystem or external services (Notion, Google Drive)
  - **Cloud access** — file upload/download/sharing capabilities for agents
  - **Document generation** — not full content but elements: images, tables, visualizations, templates LLM fills automatically
  - **Media processing** — even simple AI integrations for voice notes, photos, video add significant value
  - **Sandboxes** — dedicated environments for agents executing potentially destructive actions; consider dedicated machine for actions blocked by IP on cloud servers
  - **CLI/MCP integrations** — wrapping daily tools/services as CLI or MCP enables portability between agents, team sharing, client distribution
  - **Custom UI** — own chat interface for multi-agent collaboration or management panel; valuable when high personalization needed
- **Start small** — even one CLI/MCP integration becoming part of daily workflow teaches spotting need for more
- **Tools address clearly defined, repeatable problems** — where audio processing logic existed for years, adapt to agent context rather than rebuilding

## Technology Evolution & Feature Development (S05E03 §1-§2)

- **Gen AI duality** — stable at fundamentals, dynamic at higher layers:
  - **Stable fundamentals** (slow change): autoregression, tokenization, context window limits, hallucination, prompt injection, base knowledge cutoff — investing in logic built on these justified; expect less frequent changes
...
- **Architecture duality** — two seemingly contradictory effects simultaneously:
  ...
  - **Factory metaphor**: classic apps = building assembly line (fixed, deterministic); generative apps = building factory (agent designs own processes at runtime; you design environment)
...
- **Key heuristic**: if topic keeps returning after weeks of silence → second look; no return → skip
- **Verify personally** — industry reactions intense with heavy marketing narrative; test yourself, use experience; maintain openness and distance
...
- **Model migration considerations**:
  ...
  - Technically switching = changing model identifier; for agent systems may require **much more attention** due to cascading behavioral changes

## Agent Capability Evolution (S05E03 §4)

- **Key difference from classic apps**: in agent logic, **even small changes can significantly impact entire system's effectiveness** — unlike classic code that changes only when requirements change
- **Agent systems can continuously evolve** — driven by model improvements, new techniques, evolving tool ecosystem; changing circumstances may require **redefining established development practices**
- **Three capability levers** — expand agent capabilities without changing core architecture:
  ...
  - **Lever 2: Add tool** — e.g., terminal access transforms file editor into full dev environment; **sandbox required for dangerous tools**
...
- **Design implication**: architecture should allow capability expansion through lever changes; simple tool changes can **completely transform app's profile**

## Automatic Prompt Optimization (S05E03 §5)

- **Closed-loop prompt optimization** — autonomous systems iteratively improving prompts:
...
- **Production frameworks**: [DSPy](https://dspy.ai/) (Python) and [AX](https://axllm.dev/) (TypeScript) — prompts almost never appear in application code
  - **Signature-based approach**: declare inputs, task, expected outputs as "signatures" instead of writing prompts
  - **BootstrapFewShot**: scores traces, keeps best as demos; signature never changes, only in-context examples improve
- **Relevance to agent systems**: automatic prompt optimization can be part of agent self-improvement — agents iteratively optimizing own instructions or generating few-shot examples

## Heartbeat Pattern — Task Orchestration (S03E02 §4-§6)

- **When to use**: process too dynamic for rigid workflow but needs higher reliability than free-form agent loop
...
- **Task contract structure**: id, title, status (open/in-progress/done/waiting/blocked), agent, dependencies, capabilities, attempts, instructions
  - Contracts are **markdown files with frontmatter** — heartbeat reads as structured data
...
- **Key boundaries**:
  ...
  - **LLM only runs inside claimed task** — agents never self-select work
...
- **Human-in-the-loop**: system can request human decisions when plan requires resolution system can't make autonomously
- **Dynamic DAG extension** (S05E01 §3a) — heartbeat with agent-created task graph instead of static upfront plan:
...
- **Legal + disclosure**: when agents interact with users, explicit AI disclosure + legal protection are last line of defense for unintentional errors
...
- **Code generation + sandbox** = most impactful optimization — model writes code processing data outside context window; only compact summaries return
...
- **Key insight**: model **never sees raw data** — writes targeted code after learning schema from single sample; 11,508 records processed in ~6 turns
- **Critical caveat**: calculations in code reliable, but **data loading correctness** creates room for errors — strict supervision + code-controlled processes needed for critical documents
...
- **Five trigger types** for agent activation (unified taxonomy):
  ...
  2. **Hooks** — internal app events (e.g., subagent completion); agent resolves goal
...
- **Heartbeat pulse injection** — periodic system messages injected into main thread: "read tasks.md for pending work"
  - If no tasks require action → agent **skips** (user sees nothing; silent operation)
  - If tasks found → agent executes proactively, **resuming session on its own initiative**
- **tasks.md pattern** — file containing user-relevant activities requiring main thread context:
  - Content managed manually or by agent itself
...
- **Isolated sessions complement main thread** — webhooks and cron jobs run in separate, single-turn sessions with no shared context; prevents context pollution
...
- **Environment layer** — real-time state available to agent:
...
- **Multi-hop enrichment** — agent connects fragmentary info from multiple sources:
...
- **Background agents create business value** by connecting dispersed info — standardize and elevate process quality (marketing, sales, support)
- **Security consideration**: agent may confuse addresses/participants or include confidential info in created entries — for sensitive operations, **deterministic code should manage contacts/recipients**, not agent
...
- **Hook taxonomy** (from [AI SDK](https://ai-sdk.dev/docs/reference/ai-sdk-core/generate-text#experimental_on-tool-call-finish.on-tool-call-finish-event.output)):
  ...
  - **onStepStart / onStepFinish** — agent begins/completes step in loop
  - **onToolCallStart / onToolCallFinish** — agent begins/completes tool invocation
...
- **Hooks should have access to**: current interaction state + session info → enables event emission and direct additional actions
...
- **"Model has freedom — hooks have authority"** — model decides what to do, but hooks enforce process completion and correctness
- **Simple example**: generating session name (like ChatGPT's "New Chat" → descriptive title after first message) — hook on first user message triggers side LLM call for title generation
...
- **Instruction files per domain** — `instructions/{domain}.md`: site structure, selectors, URL patterns, scripts, known workarounds; prevents re-discovering domain each session
- **Discoveries file** — `{domain}-discoveries.md`: learned workarounds and fallbacks accumulated across sessions; forms **self-improving knowledge loop**
- **Escalating error response pattern**:
  ...
  - **Repeated failure** → save discovery (persist learning for future sessions)
- **Feedback mechanisms can be**:
  ...
  - Include instructions encouraging agent to **save lessons from errors**
...
- **Content overflow handling** — when retrieved content exceeds context limits: save to file + search, or launch subagent for overflow
- **Principle**: wherever task execution can be simplified or LLM skipped entirely, consider it — feedback should drive toward **automation of repeatable, LLM for novel**

## Human Support in Autonomous Systems (S03E03 §5)

- **Full autonomy is illusion** — real-world complexity exceeds agent capabilities even for simple, well-defined processes
- **Design for human involvement from MVP stage**:
  - **Hook-mediated checkpoints**: hooks on session start / tool call can include human verification — not just for confirming untrusted actions, but for **filling missing info** and **resolving ambiguities**
  - **Pre-completion review**: agent analyzes own actions before session end; may decide human support needed or simply inform about execution status
  - **Before/after requirements**: system defines specific human activities required (e.g., "record quality audio sample with genuine engagement")
- **"Offensive" vs "defensive" design thinking**:
  ...
  - **Offensive** (what elevates system?) — voice interface (even simple STT→TTS), richer interaction modes, proactive suggestions
...
- **Exception handling interface non-negotiable** — autonomous agents running without human involvement still need UI for: name/entity conflicts, missing data, integration errors, ambiguous inputs
- **Voice interface as upgrade path** — even simple STT→TTS interaction can dramatically increase usability for background agents needing occasional human input
- **Habit building for agent adoption** (S05E05 §12) — configuring agents not enough; must build **habit of working with them**:
  ...
  - Using MCP integration once delivers zero value; daily newsletter pointless if never read
...
- **Three compounding pressures** make AI adoption hard — skip any one and rest become fragile (S04E05 §1):
  ...
  - **Cultural** — even simple AI tools require engagement across org levels; bottom-up initiatives work best (internal workshops, experience sharing)
  - **Technical/Product** — familiar engineering extended with model selection, evaluation, agent architecture, and acknowledgment that **100% accuracy currently unlikely**
- **Query awareness gap** (S04E05 §1) — AI literacy varies even among technical users; expert users naturally include context injection ("Check Gmail and Slack, then my day plan file" = 3 tools invoked) while naive users send vague queries ("What do I have today?" = no tools invoked, empty response); query itself IS prompt
- **Four complementary adoption strategies** (S04E05 §1):
  1. **Build it right** — human in loop, human-AI cooperation, fallbacks, security by design
  2. **Lead by example** — curiosity over mandates, bottom-up adoption, discover actual needs, visible wins first
  3. **Redesign workflow** — build around AI not on top, new handoffs and ownership, feedback loops
  4. **Prove it small, scale or don't** — high-value low-risk pilots, quick wins build trust, small by design valid, measure before expanding
- **Small experiments trigger adoption cascades** (S04E05 §4) — demonstrating tool to real users in their domain immediately surfaces unanticipated needs; each demonstrated tool creates demand for next one

## Lightweight AI Tools — Documents, Prompts, and Custom Interfaces (S04E05 §2-§4)

- **AI deployment can be as simple as well-crafted document** — not every solution needs agent or custom app (S04E05 §2):
  - **Checklist** — process verification doc with AI-assisted checking (e.g., content review for SEO, internal linking, category-specific sections); significantly reduces manual oversight errors
  ...
  - **Style guide** — single prompt describing consistent visual style shared across team; simple thing with large impact on perception
- **AGENTS.md and Skills as precedent** — simple instruction sets work in programming and apply equally to non-code processes; documents/prompts gain new meaning viewed through this lens
- **When documents not enough** (S04E05 §3) — documents alone fragile because agents interpret freely; when combined with uncontrolled user queries, consider:
  - **Internal MCP server** or independent tool fully tailored to specific process
  - **AI makes building cost-effective** — even tools used briefly can be worth building; creation cost dropped dramatically
- **Content review agent pattern** (S04E05 §3) — paragraph-by-paragraph processing with add_comment tool:
  ...
  - **UI is critical differentiator** — "same effect from pasting into ChatGPT" but no convenient way to manage, accept/reject, or re-run suggestions; custom UI transforms raw AI output into workflow
  - **Design for process, not technology** — power comes from aligning with how people actually work (inline comments, accept/reject), not from underlying AI capability
