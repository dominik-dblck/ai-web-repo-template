...
Bardzo uproszczona wersja implementacji takiego systemu znajduje się w przykładzie **02_04_ops**, w której mamy **szablony agentów** posiadających dostęp do narzędzi (które dla uproszczenia zwracają statyczne dane z plików). Natomiast samo generowanie Daily Ops odbywa się już w wyniku współpracy agentów.
...
- **Dostęp do narzędzi:** są to narzędzia **delegate/message** oraz zwykle **recall/search_memory** do przeszukiwania pamięci. Jego rola i tak jest duża, więc warto unikać przeciążania go dodatkowymi narzędziami.
